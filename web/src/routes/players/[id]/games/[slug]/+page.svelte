	...
	{#if detail.lol_profile}
		...
		<section class="mb-6">
			...
			<div class="pd-cut-sm border border-[var(--color-border)] bg-[var(--color-surface-2)] p-3">
				...
				<!--
					Recent games, read from the stored history rather than from the
					blob's `recent`: the blob holds five matches and is rewritten every
					hour, so it showed the last hour, not what the member played.
				-->
				{#if detail.lol_recent?.length}
					<div class="mt-3 border-t border-[var(--color-border)] pt-3">
